...
這是一個基於 **全本地運行 (Offline-only)** 的 AI 聊天助手系統。它結合了大型語言模型 (LLM) 的邏輯推理與 VITS 的高音質語音合成，並搭配 Live2D 模型進行實時嘴型同步與角色互動。
...
-   **日語語音輸出**: 系統接收中文輸入，透過本地 LLM 生成俏皮的日文回答，並以高質量的日語 TTS 播放。
